...
def seg_text(source_text_state, init_text_queue, seg_result_queue):
    segmentor = Segmentor() #实例化分词模块
    segmentor.load(model_path) #加载分词库
    while (not init_text_queue.empty()) or source_text_state.value=='have':
        #get()方法从队头删除并返回一个项目
        #如果队列为空，get()方法就使进程阻塞timeout秒。
        #如果在timeout秒内，发现可用的项目，则继续执行。如果超时，则引发一个异常。
        try:
            source_text_list = init_text_queue.get(block=True, timeout=0.1)   
            seg_text_list = list()
            ...
            #put()方法在队尾插入一个项目
            #如果队列已满，put()方法就使子进程阻塞,直到空出一个数据单元
            seg_result_queue.put(seg_text_list, block=True, timeout=None)
        except:pass
    return

#创建一个写者进程将分词结果写入磁盘
def write_text_to_file(seg_result_queue, seg_result_file):
    with open(seg_result_file, 'w') as fw:
        while True:
            #get()方法从队头删除并返回一个项目
            #如果队列为空，get()方法就使进程阻塞,直至有项目可用
            #一个分词进程分的所有文本放进一个列表,一个列表项就是一篇文本
            text_list = seg_result_queue.get(block=True, timeout=None)
...
if __name__ == '__main__':
    '''
    为了提高分词效率，利用多进程进行分词:
    一个读者进程，负责从源文件中读取数据到初始文本队列
    多个分词进程，从初始文本队列获取文本，然后分词，将分词结果写到分词结果队列
    一个写者进程，负责将分词结果队列中的文本写入目标文件
    '''
    #--------------------------调试--------------------------------
    ...
    init_text_queue = manager.Queue(maxsize=Queue_SIZE) #创建初始文本队列
    #设置源文本标记，用来表示源文本是否读入完毕。source_text_sign是一个对象。
    source_text_state = manager.Value(None,'have')   
    segment_result_queue = manager.Queue(maxsize=Queue_SIZE) #创建分词结果队列
    
    #创建一个读者进程,将源文本读入初始文本队列。主进程不阻塞
    #python中函数传递参数：若参数是值类型则是值传递，如int、str；若参数是对象类型则是引用传递，如对象、list。
    #source_text_sign是一个对象，因此是引用传递。当引用的函数修改它时，主函数中它会相应改变，其他引用的函数中它也会相应改变。
    #因此可以用source_text_sign作为全局的标记。
    ...
    reader = multiprocessing.Process(target=read_text_to_queue, args=(source_file, init_text_queue, source_text_state))
    reader.start() #启动读者进程

    #创建一个写者进程,将分词文本从分词结果队列写入文件。主进程不阻塞
    writer = multiprocessing.Process(target=write_text_to_file, args=(segment_result_queue, segment_result_file))
    writer.start() #启动写者进程
    
    #创建进程池。一个进程池维护着一个任务队列_taskqueue。池中的processes个worker进程到任务队列中获取任务。
    pool=multiprocessing.Pool(processes=args.processes)
    
    #创建args.processes个分词进程，执行函数为 seg_text(source_text_state, initial_text_queue, seg_result_queue)。主进程不阻塞
    for i in range(args.processes):
        pool.apply_async( seg_text, (source_text_state, init_text_queue, segment_result_queue) )
    ...
    segment_result_queue.put('over', block=True, timeout=None)
